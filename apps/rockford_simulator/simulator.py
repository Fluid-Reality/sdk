"""Raw-TCP Rockford firmware simulator.

The simulator intentionally talks through the same byte stream as the hardware.
Applications connect by passing its direct ``tcp://`` endpoint to ``Rockford``.
"""
    ...
    def load(
        ...
        *,
        ...
        if not isinstance(data, dict):
            raise ValueError("Rockford board configuration must contain a JSON object")
        ...
        return cls(
            ...
            psu_voltage_v=float(data.get("psu_voltage_v", 210.0)),
...
class RockfordDeviceSimulator:
    """Connection-independent Rockford byte-protocol engine."""
    ...
    def __init__(
        ...
        *,
        ...
        state_changed: Callable[["RockfordDeviceSimulator"], None] | None = None,
        board_type: str = "rockford",
    ) -> None:
        ...
        if self.board_type != "rockford":
            raise ValueError("board_type must be 'rockford'")
...
class RockfordTcpServer:
    ...
    def __init__(
        ...
        *,
        ...
        self.last_engine: RockfordDeviceSimulator | None = None
    ...
    def _checkpoint_engine(self, engine: RockfordDeviceSimulator) -> None:
    ...
    def start(self) -> "RockfordTcpServer":
        if self._thread is not None:
            raise RuntimeError("TCP server is already running")
        self._server = TcpDeviceListener(self.host, self.port).start()
        self._thread = threading.Thread(target=self.serve_forever, name="rockford-tcp-server", daemon=True)
        self._thread.start()
        return self
    ...
    def serve_forever(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                engine = RockfordDeviceSimulator(
    ...
    def __enter__(self) -> "RockfordTcpServer":
        return self.start()
...
def print_connection_instructions(endpoint: str) -> None:
    ...
    print(f'Python: Rockford("{endpoint}")')
...
def run_tcp_simulator(
    ...
    *,
    ...
    if board_type is None:
        ...
        board_type = str(design.get("board_type", "rockford")).lower()
    if board_type != "rockford":
        raise ValueError("Saved board_type must be 'rockford'")
    with RockfordTcpServer(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a Rockford device over its raw TCP byte stream")
    parser.add_argument("config", type=Path, help="Single-file Rockford board JSON")
    parser.add_argument(
        "--board",
        choices=("rockford",),
