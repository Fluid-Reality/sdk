...
- View actuators by bank: group 0 shows `0-7`, group 1 shows `8-15`, and group 2 shows `16-23`.
...
- When PSU is on and output is connected, the selected group is auto-detected:
  ...
  - delta `> 3.0 mA` means error and actuator actions are blocked
  - otherwise the actuator is shown as `Ready` and available
- Connected actuators expose a configurable `Recover` action, including working and error-state actuators. Recovery temporarily disables manual-output safety, alternates raw output between positive and negative manual drive for the requested duration, reports the current delta every second, restores safety, and reports the final delta against baseline.
